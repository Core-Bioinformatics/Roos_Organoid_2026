...
metadata_cells <- readRDS(file.path(obj_path, "cao_df_cell.RDS"))
...
for (organ in target_organs) {
    ...
    qs::qsave(so, file.path(obj_path, paste0("cao_", organ, "_filtered.qs")), nthreads = 30)
...
for (organ in target_organs) {
    so <- so_objects[[organ]]
    so <- subset(so, nFeature_RNA > 1000 & nCount_RNA < 5e3 & percent_rp < 10)
    print(ncol(so))
    Idents(so) <- "Organ"
    qs::qsave(so, file.path(obj_path, paste0("cao_", organ, "_filtered.qs")), nthreads = 30)
    so_objects[[organ]] <- so
...
for (organ in target_organs) {
    ...
    print(ncol(so))
    Idents(so) <- "Organ"
    print(VlnPlot(so, features = mtd_rna, ncol = 3, pt.size = 0, log = FALSE)) +
...
for (organ in target_organs) {
    ...
    qs::qsave(so, file.path(obj_path, paste0("cao_", organ, "_filtered_normalized.qs")), nthreads = 30)
}
