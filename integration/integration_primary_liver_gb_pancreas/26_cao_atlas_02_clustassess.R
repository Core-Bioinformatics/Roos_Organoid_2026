...
chosen_batch_correction <- "harmony"
chosen_theta <- 2 # try with theta = 30, 50 as well
...
mtd_configs <- read.csv(file.path(mtd_folder, "26_cao_clustassess_configs.csv"), header = TRUE, comment.char = "#")
nreps <- 100
...
for (i in seq_len(nrow(mtd_configs))) {
    id <- mtd_configs$id[i]
    so_path <- file.path(objects_folder, paste0(id, "_filtered_normalized.qs"))
